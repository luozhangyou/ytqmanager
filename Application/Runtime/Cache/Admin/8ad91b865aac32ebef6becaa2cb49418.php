...
$(function(){
	...
	$("#"+tableList).datagrid({
		...
		url:'/taoyongjin/Admin/User/pageList',
		fitColumns:true,
		nowrap:true,
		selectOnCheck:false,
		animate:true,
		...
		iconCls: 'fa fa-easyui fa-plus-square',
				...
				if(null==selectedRow){
					...
					$.messager.confirm('确认操作', '初始化密码为：123456 ?', function(r){
						if (r){
							...
							ajaxReturnList("/taoyongjin/Admin/User/initPassword",o);
						}
					});
				}
			}
			...
			handler: function(){
				...
				if(null==selectedRow){
					...
					var rows=ajaxReturnList("/taoyongjin/Admin/User/getUserRole",o);
					var node=null;
			...
			handler: function(){
				...
				if(null==selectedRow){
					...
					ajaxDelRowsDatagrid('/taoyongjin/Admin/User/delRows',ids,tableList);
				}
			}
		},'-',{
	...
	$("#"+saveBtn).click(function(){
		ajaxSubmitForm(auForm,'/taoyongjin/Admin/User/addOrUpdate',addDiv,tableList);
	});
	...
	$("#saveBtn_setRoles_user").click(function(){
    	...
    	var obj=ajaxReturnList("/taoyongjin/Admin/User/setRoles",o);
    	if(obj.flag){
    		$("#setRolesDiv_user").window("close");
    	}
	...
	$('#roleTree_user').tree({
		url:'/taoyongjin/Admin/Role/getTree',
		editable:true,
		checkbox:true,
		cascadeCheck:false,
